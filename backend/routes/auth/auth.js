...
const flog = logger.child({fileContext: 'auth'});

const speakeasy = require('speakeasy');
const qrcode = require('qrcode');
const {encrypt, decrypt} = require('./crypto.js');
const {deleteOldAvatar} = require('../profile/save_avatar.js');

const tempSetupSecret = new Map();

async function registerUser(fastify, options)
{
    const {secure, DBinsert} = options;
    fastify.post(API_PROTOCOL.REGISTER_USER.path, {schema: signSchema}, async (request, reply) => {
        ...
        catch (err)
        {
            ...
            return reply.code(status).send({
                error: err && err.error ? err.error : 'REGISTER_FAILED',
                message: err && err.message ? err.message : 'Could not register user'
            });
        }
    });
}
async function loginUser(fastify, options)
{
    ...
    fastify.post(API_PROTOCOL.LOGIN_USER.path, async (request, reply) => {
        ...
        try
        {
            ...
            if (is2faEnabled)
            {
                const tempToken = secure.generateTemporaryToken({id: result.id, username, type: '2fa_pending'});
            ...
            secure.setAuthCookie(reply, token);
            return reply.code(200).send({status: 'LOGGED_IN'});
        }
        catch (err)
        {
...
async function logoutUser(fastify, options)
{
    ...
    fastify.post(API_PROTOCOL.LOGOUT_USER.path, {}, async (request, reply) => {
        ...
        try
        {
            ...
            await DBupdate.updateOnlineStatus(userId, false);
    ...
    fastify.delete(API_PROTOCOL.DELETE_PROFILE.path, async (request, reply) => {
        ...
        try
        {
            ...
            try
            {
                ...
                oldAvatarUrl = user && user.avatar_file ? user.avatar_file : null;
            }
            catch (err) {}
            ...
            if (result === 1)
            {
                try
                {
                    if (oldAvatarUrl && typeof oldAvatarUrl === 'string' && oldAvatarUrl.startsWith('api/avatars/'))
                        await deleteOldAvatar('api/avatars/');
                }
                catch (err) {}
                secure.clearAuthCookie(reply);
