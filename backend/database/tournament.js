...
async function updateTournamentStats(db, gameId, p1score, p2score, status, winnerId) {
	const { run } = _wrap(db);
	return run(
		`UPDATE games
     SET p1_score = ?, p2_score = ?, status = ?, winner_id = ?
     WHERE id = ?`, [p1score, p2score, status || 'finished', winnerId, gameId]
	);
}
...
async function updateBracket(db, tournamentId, winnerId, round) {
	...
	const nextGame = await get(
		`SELECT id, p1_id, p2_id
			FROM games
		 WHERE tournament_id = ?
		 	AND round = ?`, [tournamentId, nextRound]
	);
	if (!nextGame) {
		await run(`UPDATE tournaments SET winner_id = ?, status = 'finished' WHERE id = ?`, [winnerId, tournamentId]);
		return { gameId: null, slot: null };
	}
	let slot = null;
	if (!nextGame.p1_id) {
		slot = 'p1_id';
